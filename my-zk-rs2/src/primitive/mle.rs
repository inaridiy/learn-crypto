...
/// $\widetilde{eq}(x, p) = \prod_i \left( x_i p_i + (1 - x_i)(1 - p_i) \right)$
#[inline]
pub fn teq<F: Field, const N: usize>(point: &[F; N]) -> MvPolynomial<F, N> {
    let mut eq = MvPolynomial::one();

    for i in 0..N {
        let x = MvPolynomial::variable(i);
        let factor =
            x.clone().scale(point[i]) + (MvPolynomial::one() - x).scale(F::one() - point[i]);
        eq *= factor;
    }
...
impl<const N: usize> BoolPoint<N> {
    ...
    pub fn teq<F: Field>(&self) -> MvPolynomial<F, N> {
        teq(&self.to_field_point())
    }

    /// $eq_w(\mathrm{point})$ の値。$w$ はこのブール点。
    #[inline]
    ...
    [(); 1 << N]:,
{
    ...
    /// 各ブール点 $w$ について equality polynomial $eq_w(\mathrm{point})$ をテーブル順に並べる。
    #[inline]
    ...
    [(); 1 << N]:,
{
    ...
    /// 定義通りに $\tilde{f}(x) = \sum_w f(w) \cdot eq_w(x)$ として MLE を構成する。
    #[inline]
...
mod tests {
    ...
    #[test]
    fn eq_polynomial_extends_boolean_equality() {
        let point = [f(2), f(3)];
        let eq = teq(&point);

        // ブール点 w での評価は eq_w(point) と一致する (eq̃ の対称性)。
        for bool_point in BoolHyperCube::<2>::iter() {
            assert_eq!(
                eq.eval(&bool_point.to_field_point()),
                bool_point.eq_eval(&point)
            );
        }
    }
