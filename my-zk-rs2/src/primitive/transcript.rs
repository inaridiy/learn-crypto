...
impl Transcript {
    ...
    /// 多項式を項の列 `(exponents, coeff)` として transcript に追加する。
    #[inline]
    pub fn append_polynomial<F, const N: usize>(&mut self, label: &[u8], poly: &MvPolynomial<F, N>)
    where
        F: Field + CanonicalSerialize,
    {
        ...
        for (monomial, coeff) in poly.terms() {
            for exp in monomial.exponents() {
                self.append_usize(b"poly-monomial-exp", *exp);
            }
            self.append_field(b"poly-coeff", coeff);
        }
    }
    ...
    /// 同じラベルで `N` 個の field challenge を順番に取り出す。
    ///
    /// challenge を取り出すたびに transcript が進むので、`N` 個はすべて異なる値になる。
    #[inline]
    pub fn challenge_field_array<F, const N: usize>(&mut self, label: &[u8]) -> [F; N]
    where
        F: PrimeField + CanonicalSerialize,
    {
        let mut challenges = [F::zero(); N];
        for challenge in challenges.iter_mut() {
            *challenge = self.challenge_field(label);
        }
        challenges
    }
}
